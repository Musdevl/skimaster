package fr.univcotedazur.skimaster.order.controllers;
...
import static fr.univcotedazur.skimaster.order.controllers.OrderController.BASE_URI;
...
public class OrderController {
    ...
    public static OrderDTO convertOrderToDto(Order order) { // In more complex cases, we could use a ModelMapper such as MapStruct
        return new OrderDTO(order.getId(), order.getCustomerId(), order.getPrice(), order.getPayReceiptId(), order.getStatus());
    }
