package fr.univcotedazur.skimaster.order.repositories;
...
class OrderRepositoryTest {
    ...
    @BeforeEach
    void setup() {
        Customer john = customerRepository.save(new Customer("john", "1234567890", Category.ADULT));
        johnId = john.getId();
        Order createdOrder = new Order(john, new HashSet<>(List.of(new Item(Plan.BASIC_PLAN, 2))), 20, "payReceiptIdOK");
    ...
    void testEmptyPriceAtOrderCreation() {
        ...
        Order orderWithZeroPrice = new Order(john, new HashSet<>(List.of(new Item(Plan.BASIC_PLAN, 5))), 0, "payReceiptIdOK");
        ...
        Order orderWithEmptyReceiptId = new Order(john, new HashSet<>(List.of(new Item(Plan.BASIC_PLAN, 5))), 50, "");
...
//                Sort.by(Sort.Direction.DESC,"id")).size());
        ...
        Assertions.assertEquals(Plan.BASIC_PLAN,retrievedOrder.getItems().stream().findFirst().get().getPlan());
...
//                Sort.by(Sort.Direction.DESC,"id")).size());
    ...
    void testAllOrdersByPageable() {
        ...
        Order secondOrder = new Order(john, new HashSet<>(List.of(
                new Item(Plan.BASIC_PLAN,5))), 50.0, "payReceiptIdOK");
